...
try:
    ...
    print 'open the source successfuly,start reading...'
    content = buf.read()
finally:
    buf.close()
...
try:
    ...
    conn = MySQLdb.connect(host="localhost",user="root",passwd=password,db="data2000",charset='utf8')
    cur = conn.cursor()
...
#         param = tuple(record)
    ...
    conn.close()
    log = open('log.txt','a')
    log.write(time.ctime()+'--'+dataf[index]+'--counter:<'+str(count)+'>records\n')
    log.close()
    print 'mysql conn closed,over'